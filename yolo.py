...
def draw_labels(boxes, confs, colors, class_ids, classes, img): 
    ...
    for i in range(len(boxes)):
        if i in indexes:
            ...
            confidence = confs[i]  # get the confidence score
            color = colors[i]
            cv2.rectangle(img, (x,y), (x+w, y+h), color, 2)
            cv2.putText(img, f"{label} {confidence:.2f}", (x, y - 5), font, 1, color, 1)  # include confidence score
            if label == 'Fire' and not beeped:
